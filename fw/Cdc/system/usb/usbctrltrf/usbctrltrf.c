...
{   
    if(USTAT == EP00_OUT)
    {
        if(ep[0].out.a.Stat.c.PID == SETUP_TOKEN)       // EP0 SETUP
...
void USBCtrlTrfOutHandler(void)
{
    if(ctrl_trf_state == CTRL_TRF_RX)
    {
         ...
         */
        if(ep[0].out.a.Stat.a.DTS == 0)
            ep[0].out.a.Stat._byte = _USIE|_DAT1|_DTSEN;
        else
            ep[0].out.a.Stat._byte = _USIE|_DAT0|_DTSEN;
    }
    else    // CTRL_TRF_TX
...
void USBCtrlTrfInHandler(void)
{
    ...
    if(ctrl_trf_state == CTRL_TRF_TX)
    {
        ...
        if(ep[0].in.a.Stat.a.DTS == 0)
            ep[0].in.a.Stat._byte = _USIE|_DAT1|_DTSEN;
        else
            ep[0].in.a.Stat._byte = _USIE|_DAT0|_DTSEN;
    }
    else // CTRL_TRF_RX
        USBPrepareForNextSetupTrf();
...
{    
    ...
    ep[0].in.a.Stat.a.BC9 = 0;
    ep[0].in.a.Stat.a.BC8 = 0;
    ep[0].in.a.Stat._byte |= MSB(byte_to_send);
    ep[0].in._byte[1] = LSB(byte_to_send);
    ...
    if(usb_stat.a.ctrl_trf_mem == _ROM)       // Determine type of memory source
    {
    ...
    }//end if(usb_stat.a.ctrl_trf_mem == _ROM)
 ...
 *                  usb_stat.a.ctrl_trf_mem is always _RAM.
...
void USBCtrlTrfRxService(void)
{
    ...
    MSB(byte_to_read) = 0x03 & ep[0].out.a.Stat._byte;    // Filter out last 2 bits
    LSB(byte_to_read) = ep[0].out.a.Cnt;
...
void USBCtrlEPServiceComplete(void)
{
    if(ctrl_trf_session_owner == MUID_NULL)
    {
         ...
         */
        ep[0].out.a.Cnt = EP0_BUFF_SIZE;
        ep[0].out.b.ADR = (byte*)&SetupPkt;
        
        ep[0].out.a.Stat._byte = _USIE|_BSTALL;
        ep[0].in.a.Stat._byte = _USIE|_BSTALL;
    }
    else    // A module has claimed ownership of the control transfer session.
    {
        if(SetupPkt.d.DataDir == DEV_TO_HOST)
        {
            ...
            ep[0].out.a.Cnt = EP0_BUFF_SIZE;
            ep[0].out.b.ADR = (byte*)&SetupPkt;            
            ep[0].out.a.Stat._byte = _USIE;           // Note: DTSEN is 0!
        ...
        else    // (SetupPkt.d.DataDir == HOST_TO_DEV)
        {
            ...
            ep[0].out.a.Stat._byte = _USIE|_DAT1|_DTSEN;
        }//end if(SetupPkt.d.DataDir == DEV_TO_HOST)
...
void USBPrepareForNextSetupTrf(void)
{
    ...
    ep[0].out.a.Cnt = EP0_BUFF_SIZE;              // Defined in usbcfg.h
    ep[0].out.b.ADR = (byte*)&SetupPkt;
    ep[0].out.a.Stat._byte = _USIE|_DAT0|_DTSEN;  // EP0 buff dsc init, see usbmmap.h
    ep[0].in.a.Stat._byte = _UCPU;               // EP0 IN buffer initialization
