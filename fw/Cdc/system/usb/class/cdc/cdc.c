...
void USBCheckCDCRequest(void)
{
     ...
     */
    if(SetupPkt.d.Recipient != RCPT_INTF) return;

    /*
     * If request type is not class-specific then return
     */
    if(SetupPkt.d.RequestType != CLASS) return;

    /*
     * Interface ID must match interface numbers associated with
     * CDC class, else return
     */
    if((SetupPkt.h.bIntfID != CDC_COMM_INTF_ID)&&
       (SetupPkt.h.bIntfID != CDC_DATA_INTF_ID)) return;
    ...
    switch(SetupPkt.b.bRequest)
    {
        ...
        case SET_CONTROL_LINE_STATE:
            ctrl_trf_session_owner = MUID_CDC;
            control_signal_bitmap._byte = LSB(SetupPkt.c.W_Value);
            break;
...
void CDCInitEP(void)
{
    ...
    line_coding.b.dwDTERate._dword = 115200;     // baud rate
    line_coding.b.bCharFormat = 0x00;            // 1 stop bit
    line_coding.b.bParityType = 0x00;            // None
    line_coding.b.bDataBits = 0x08;              // 5,6,7,8, or 16
     ...
     *          sent.
     */
    CDC_INT_BD_IN.b.ADR = (byte*)cdc_notice;     // Set buffer address
    CDC_INT_BD_IN.a.Stat._byte = _UCPU|_DAT1;     // Set status

    CDC_BULK_BD_OUT.a.Cnt = sizeof(cdc_data_rx);  // Set buffer size
    CDC_BULK_BD_OUT.b.ADR = (byte*)cdc_data_rx;  // Set buffer address
    CDC_BULK_BD_OUT.a.Stat._byte = _USIE|_DAT0|_DTSEN;// Set status

    CDC_BULK_BD_IN.b.ADR = (byte*)cdc_data_tx;   // Set buffer size
    CDC_BULK_BD_IN.a.Stat._byte = _UCPU|_DAT1;    // Set buffer address
...
byte getsUSBUSART(char *buffer, byte len)
{
    ...
    if(!mCDCUsartRxIsBusy())
    {
         ...
         */
        if(len > CDC_BULK_BD_OUT.a.Cnt)
            len = CDC_BULK_BD_OUT.a.Cnt;
         ...
         */
        CDC_BULK_BD_OUT.a.Cnt = sizeof(cdc_data_rx);
 ...
 *                  See CDCTxService() for more details.
 *****************************************************************************/
void putrsUSBUSART(const_rom char *data)
{
    byte len;
...
void CDCTxService(void)
{
    ...
    else if(cdc_trf_state == CDC_TX_BUSY)
    {
         ...
         */
        CDC_BULK_BD_IN.a.Cnt = byte_to_send;
        ...
        if(cdc_tx_len == 0)
        {
            if(CDC_BULK_BD_IN.a.Cnt == sizeof(cdc_data_tx))
